import Parse, { Cloud, User } from 'parse';
import cloud from '../cloud';

type TAction = 'login' | 'register';

export const getAuthLink = async (action: TAction) => {
  try {
    const res = await Cloud.run(cloud.api.getAuthLink.name, { action: action });
    return res;
  } catch (error) {
    return;
  }
...
export const registerWithGoogle = async (params: any): Promise<{ email: string }> => {
  try {
...
export const resetPasswordService = async (email?: string) => {
  if (email) return await User.requestPasswordReset(email);
  else {
