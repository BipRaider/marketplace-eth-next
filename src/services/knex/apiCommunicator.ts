...
const resultToString = async (res: Response) => (res.status === 200 ? '' : (await res.text()) ?? `Error ${res.status}`);
...
export const apiEditUser = async (idToEdit: number, propertiesToEdit: Partial<User>, _delete: boolean = false) => {
...
export const apiLikeArticle = async (articleId: number, like: boolean) => {
  ...
  return res.status === 200 ? ((await res.json()) as { liked: boolean; likes: number }) : null;
};
...
export const apiGameReward = async (hints: number, switches: number) => {
  ...
  return res.status === 200 ? ((await res.json()) as { currentPoints: number; pointsReceived: number }) : null;
};
